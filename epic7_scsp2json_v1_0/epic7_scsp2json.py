'''
  link for me - http://en.esotericsoftware.com/spine-binary-format
...
import codecs, os, pathlib, sys

import _epic7_scsp2json_check as _check
...
argv_minify    = _scsp_funcs.argv_option_check("-minify")
argv_wtempfile = _scsp_funcs.argv_option_check("-wtempfile")
argv_e7herder  = _scsp_funcs.argv_option_check("-e7herder")

aa = argv_minify + argv_wtempfile + argv_e7herder + 1

if len(sys.argv) > (aa):
  file4convert = sys.argv[aa]
  
  if len(sys.argv) > (aa + 1):
    filename4json = sys.argv[aa + 1]
  else:
    filename4json = file4convert + ".json"
    
else:
  print("Usage: python epic7_scsp2json.py [optional: options] <.scsp file for convert to json> <optional: filename for new .json file (result)>\n\toptions:\n\t-minify\n\t-wtempfile\n\t-e7herder")
  sys.exit(1)

_check.scsp2json_start(file4convert, filename4json, argv_minify, argv_wtempfile, argv_e7herder)
